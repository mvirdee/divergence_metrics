...
# for temporal aggregation method 3
##############################################################################
...
for i, t in enumerate(t_subset_sizes):
    reduced_t_max = t_max-t
    ref_groups=[] # create list of subsets for current value of t
    mod_groups=[]
    ad_ref_groups=[]
    
    print("Subset size: {}, number of subsets: {}".format(t, n_samples[i]))
    for n in range(n_samples[i]):
        # randomly select start times of n subsets of length
        t_st = np.random.randint(t_min, reduced_t_max)

        rg = ref.isel(time=slice(t_st, t_st+t))
        ref_groups.append(rg)

        mg = mod.isel(time=slice(t_st, t_st+t))
        mod_groups.append(mg)

        arg = ad_ref.isel(time=slice(t_st, t_st+t))
        ad_ref_groups.append(arg)

    ref_grouped[t] = ref_groups
    mod_grouped[t] = mod_groups
    ad_ref_grouped[t] = ad_ref_groups
...
for j, v in enumerate(var_list):
    ...
    for i, model in enumerate(models_list_s+["MERRA2"]):
        ...
        for t in t_subset_sizes:
            print("Subset size: {}, number of subsets: {}".format(t, int(np.ceil(t_max/t))))
            # print("Number of time subsets: {}, ", math.ceil(t_max/t))
            ref_groups = [g for g in ref_grouped[t]]
            mod_groups = [g for g in mod_grouped[t]]

            ad_ref_groups = [g for g in ad_ref_grouped[t]]
...
for i, df in enumerate([results_df, results_Lmoms_df, results_Lmoms_ref_df]):
    filename = 'results/'+region.replace(' ', '')+"_"+df_names[i]+'_temporal_method3.parquet'
    print(filename)
